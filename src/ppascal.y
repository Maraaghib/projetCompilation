...
E: E Pl E {$$ = create_noeud($1,$3,"Pl",0,creer_type(0,Pl));}
| E Mo E {$$ = create_noeud($1,$3,"Mo",0,creer_type(0,Mo));}
| E Mu E {$$ = create_noeud($1,$3,"Mu",0,creer_type(0,Mu));}
| E Or E {$$ = create_noeud($1,$3,"Or",0,creer_type(0,Or));}
| E Lt E {$$ = create_noeud($1,$3,"Lt",0,creer_type(0,Lt));}
| E Eq E {$$ = create_noeud($1,$3,"Eq",0,creer_type(0,Eq));}
| E And E {$$ = create_noeud($1,$3,"And",0,creer_type(0,And));}
| Not E {}
| PO E PF {}
| I {$$ = create_noeud(NULL,NULL,$1,T_int,creer_type(0,T_int));}
| V {ENV pos = rech2($1,env_cour->debut,env_global->debut);
   if(pos == NULL){
     yyerror("Variable non déclarée");
     exit(EXIT_FAILURE);
   }
   $$ = create_noeud(NULL,NULL,pos->ID,pos->typeno->TYPEF,pos->typeno);}
| True {$$ = create_noeud(NULL,NULL,"true",True,creer_type(0,True));}
| False {$$ = create_noeud(NULL,NULL,"false",False,creer_type(0,False));}
...
C: C Se C {$$ = create_noeud($1,$3,"Se",0,creer_type(0,Se));}//juste
| Et Af E {$$ = create_noeud($1,$3,"Af",0,creer_type(0,Af));} //juste
| V Af E {ENV pos = rech2($1,env_cour->debut,env_global->debut);
   if(pos == NULL){
     yyerror("Variable non déclarée");
     exit(EXIT_FAILURE);
   }
   $$ = create_noeud(create_noeud(NULL,NULL,pos->ID,pos->typeno->TYPEF,pos->typeno),$3,"AF",0,creer_type(0,Af));
   // juste
 }
| Sk {printf("Sk\n");}
| AO C AF {} //{ C }
| If E Th C El C {$$ = create_noeud($2,create_noeud($4,create_noeud($6,NULL,"El",El,creer_type(0,El)),"Th",Th,creer_type(0,Th)),"If",If,creer_type(0,If));}
| Wh E Do C {}
...
L_argsnn: E {$$ = $1;}
| E Virgule L_argsnn {$$ = create_noeud($1,$3,"Virgule",Virgule,creer_type(0,Virgule)); }
  ;
...
D_entp: Dep V PO L_argt PF /*Dep NPro ( L_argt )*/{env_global = env_cour;
   ...
   inbilenv(env_cour,$2,creer_type(0,Dep));/*Ajout du nom de la procedure dans les vars locals*/}
;
