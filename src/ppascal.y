...
MP: L_vart LD C {
                  env_global = $1;
                  syntree = create_noeud($3, NULL, "Mp", Mp, creer_type(0,Mp));
                  //ecrire_bilenv($1);
...
E: E Pl E {$$ = create_noeud($1,$3,"Pl",Pl,creer_type(0,Pl));}
| E Mo E {$$ = create_noeud($1,$3,"Mo",Mo,creer_type(0,Mo));}
| E Mu E {$$ = create_noeud($1,$3,"Mu",Mu,creer_type(0,Mu));}
| E Or E {$$ = create_noeud($1,$3,"Or",Or,creer_type(0,Or));}
...
C: C Se C {$$ = create_noeud($1,$3,"Se",Se,creer_type(0,Se));}//juste
| Et Af E {$$ = create_noeud($1,$3,"Af",Af,creer_type(0,Af));} //juste
| V Af E {ENV pos = rech2($1,env_cour->debut,env_global->debut);
   if(pos == NULL){
     yyerror("Variable non déclarée");
     exit(EXIT_FAILURE);
   }
   $$ = create_noeud(create_noeud(NULL,NULL,pos->ID,pos->typeno->TYPEF,pos->typeno),$3,"AF",Af,creer_type(0,Af));
   // juste
 }
| Sk {printf("Sk\n");}
| AO C AF {} //{ C }
